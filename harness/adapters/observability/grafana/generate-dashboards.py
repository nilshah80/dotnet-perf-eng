...
def flamegraph(pid, title, gp, label_selector, desc=None):
    p = _base(pid, title, gp, "flamegraph", datasource=PYRO, desc=desc)
    p["targets"] = [{
        "datasource": PYRO,
        "refId": "A",
        "queryType": "profile",
    ...
    p["options"] = {"showHistogram": True}
    return p


def markdown(pid, title, gp, content, desc=None):
    p = {
        "id": pid,
        "title": title,
        "type": "text",
...
# ===========================================================================
# Dashboard 50 -- Continuous CPU profiling (Pyroscope)
# ===========================================================================
def build_profiling(s):
    g = Grid()
    selector = '{service_name=~"$service",perf_run_id=~"$run"}'
    g.add_row("CPU flame graph (opt-in continuous profiling)")
    g.place(flamegraph(None, "Process CPU", {}, selector,
                       desc="Requires PERFLAB_CONTINUOUS_PROFILING=1. Queries process_cpu over the dashboard time range for $service and $run. Allocation, heap, lock, and exception profiling stay off."), 24, 16)
    g.add_row("Explore")
    g.place(markdown(None, "Profiles Drilldown / Explore", {},
                     """Continuous CPU profiling is **opt-in** (`PERFLAB_CONTINUOUS_PROFILING=1` or PerfLab `--continuous-profiling`) and perturbs the process. Hold the same setting on baseline and candidate.
...
""",
                     desc="Deep-link into Grafana Profiles Drilldown for the selected service and run."), 24, 8)
    tv = templating(s["app"], s["service_regex"])
    return "50-profiling.json", dashboard(f'{s["uid"]}-profiling',
                                          f'{s["human"]} · CPU Profiling', g.panels, tv,
                                          ["perflab", s["uid"], "profiling"])


# ===========================================================================
# Datasource overlay -- signal correlation in one Grafana.
# grafana/otel-lgtm ships Loki -> Tempo (trace_id derived field), Tempo -> Loki
# (tracesToLogsV2) and Prometheus exemplars -> Tempo, but NOT Tempo -> Pyroscope.
# This file REPLACES the image's grafana-datasources.yaml (same names/UIDs, so
# every dashboard and PerfLab deep link keeps working) and adds tracesToProfiles
# and tracesToMetrics. NOTE (verified against Grafana 13 source,
# createSpanLink.tsx): Grafana renders the span-level profile button only when
# the span carries a pyroscope.profile.id tag, which requires the
# Pyroscope.OpenTelemetry span-profiles SDK inside the app (deliberately not
# adopted). The block is pre-configured so the button appears if a lab app ever
# adopts it; until then trace -> profile is Explore Split -> Pyroscope with the
# shared time range. tracesToMetrics ("Request rate") works today.
# ===========================================================================
DATASOURCES_YAML = """# GENERATED by harness/adapters/observability/grafana/generate-dashboards.py -- do not edit.
# Mounted over /otel-lgtm/grafana/conf/provisioning/datasources/grafana-datasources.yaml.
# Same names/UIDs as grafana/otel-lgtm; adds Tempo -> Pyroscope (tracesToProfiles).
apiVersion: 1
  ...
  - name: Tempo
    ...
    jsonData:
      ...
      tracesToMetrics:
        ...
        queries:
          - name: "Request rate"
            query: 'sum(rate(http_server_request_duration_seconds_count{$$__tags}[$$__rate_interval]))'
      serviceMap:
        datasourceUid: "prometheus"
      search:
        hide: false
...
def write_datasources(lab):
    outdir = os.path.join(REPO_ROOT, "labs", lab, "infra", "grafana")
    path = os.path.join(outdir, "datasources.yaml")
    with open(path, "w", encoding="utf-8") as f:
        f.write(DATASOURCES_YAML % {"profile_type": "process_cpu:cpu:nanoseconds:cpu:nanoseconds"})
    return path
...
def main():
    for lab, s in LABS.items():
        ...
        builders = [build_overview, build_runtime, build_http, build_dependencies, build_profiling]
        ...
        print(f"    datasources.yaml -> {write_datasources(lab)}")
        for w in written:
            print(f"    {w}")
