...
#
#   trend-report.sh [--lab NAME] [--scenario ID] [--metric NAME] [--profile P] [--generator G] [--last N]
#
...
lab="${PERFLAB_PROJECT:-}"; scenario=""; metric="http.latency.p99"; profile=""; generator=""; last="0"; include_partial="false"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --lab) lab="${2:?}"; shift 2 ;;
...
rows="$(jqd -r --arg s "${scenario}" --arg m "${metric}" --arg p "${profile}" --arg g "${generator}" --argjson incp "${incp}" '
  (.captureStatus // "unknown") as $st
  | select( ($s=="" or .scenarioId==$s) and ($p=="" or .profile==$p) and ($g=="" or (.loadGenerator // "")==$g)
            and (.metrics[$m] != null) and ($incp or $st=="captured") )
  | [ (.ts|sub("T";" ")|sub("Z";"")), .gitRevision, (if .gitDirty then "*" else "" end), .scenarioId, .profile, (.metrics[$m]|tostring), $st, (.loadGenerator // "?") ] | @tsv' < "${file}")"
if [[ -z "${rows}" ]]; then
...
echo "Perf trend -- lab=${lab} metric=${metric}$([[ -n "${scenario}" ]] && echo " scenario=${scenario}")$([[ -n "${profile}" ]] && echo " profile=${profile}")$([[ -n "${generator}" ]] && echo " generator=${generator}")$([[ "${include_partial}" == "true" ]] && echo " (incl. partial)")"
printf '%s\n' "${rows}" | awk -F'\t' '
  { v[NR]=$6+0; ts[NR]=$1; rev[NR]=$2; dirty[NR]=$3; scn[NR]=$4; prof[NR]=$5; st[NR]=$7; gen[NR]=$8;
    if (!(($4 SUBSEP $5 SUBSEP $8) in seen)) { seen[$4 SUBSEP $5 SUBSEP $8]=1; groups++ }
    flag[NR]=($7=="captured"?"":"!"); if($7!="captured") partial++; if ($6+0>mx) mx=$6+0 }
  END{
    printf "  %-19s %-9s %-8s %-8s %-7s %-12s %-9s %s\n","date","commit","scenario","profile","gen","value","delta","";
    for(i=1;i<=NR;i++){
      d = (i>1) ? v[i]-v[i-1] : 0;
      dp = (i>1 && v[i-1]!=0) ? sprintf("%+.1f%%", 100*d/v[i-1]) : "-";
      barlen = (mx>0) ? int(30*v[i]/mx) : 0; bar=""; for(j=0;j<barlen;j++) bar=bar"#";
      printf "  %-19s %-7s%-2s %-8s %-8s %-7s %-12.4g %-9s %s\n", ts[i], rev[i], (dirty[i] flag[i]), scn[i], prof[i], gen[i], v[i], dp, bar;
    }
    ...
    # A delta between a steady and a ramp run, or a k6 and a JMeter run, is not a
    # regression. Say so rather than chart it as one.
    if (groups>1) printf "  WARNING: rows mix %d scenario/profile/generator combinations; deltas across them are not comparisons (narrow with --scenario, --profile, --generator).\n", groups;
  }'
