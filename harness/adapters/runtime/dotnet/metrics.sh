...
PERFLAB_METRIC_ROLES=(
  'process_cpu|range|rate(dotnet_process_cpu_time_seconds_total{job=~"$JOB"}[1m])'
  'working_set|range|dotnet_process_memory_working_set_bytes{job=~"$JOB"}'
  'gc_heap|range|dotnet_gc_last_collection_heap_size_bytes{job=~"$JOB"}'
  'thread_pool_queue|range|dotnet_thread_pool_queue_length_total{job=~"$JOB"}'
  'request_duration|range|http_server_request_duration_seconds_count{job=~"$JOB"}'
  'gc_allocation_rate|range|rate(dotnet_gc_heap_allocated_bytes_total{job=~"$JOB"}[1m])'
  'gc_committed|range|dotnet_gc_last_collection_memory_committed_size_bytes{job=~"$JOB"}'
  'gc_collections|range|dotnet_gc_collections_total{job=~"$JOB"}'
  'gc_pause|range|rate(dotnet_gc_pause_time_seconds_total{job=~"$JOB"}[1m])'
