...
| ID | Item | Note |
| --- | --- | --- |
| D-P1-3 | Span-to-profile correlation | Landed with no application change: CPU profiles only, x64 only (pyroscope-dotnet's managed span API refuses other architectures and reads `DD_PROFILING_ENABLED`, which the injection sets). The deploy-time injection (`harness/adapters/runtime/dotnet/injection`, a `DOTNET_STARTUP_HOOKS` assembly the lab compose files stage through an init container) runs `PyroscopeSpanProcessor`'s own hooks from an `ActivityListener`, so each local root span carries `pyroscope.profile.id` and its CPU samples the same id. Both products ask Pyroscope's span-profile API for exactly the captured detail traces' spans, over the window widened by 60 s because an upload is stamped at the start of its period (`span-profiles.json`, best-effort, never required). Proven live on x64: 126 ms of CPU on the four detail spans (native `harness/adapters/observability/grafana/capture-span-profiles-test.sh`, `harness/adapters/runtime/dotnet/injection/injection-test.sh`; `go test ./plugins/apm/grafana/ -run SpanProfile`) |
...
| D-P1-8 | Dynamic `perf.phase` and run baggage | Landed as `perflab-baggage-v1` with no application change. Every generator sends `baggage: perf.run.id=…,perf.phase=…` (k6 `baggage.js`, `wrk.lua`, the JMeter plans, the closed distributed script; values the application would reject are not sent). The injection's hosting-startup middleware stamps them on the request span and log scope and the phase on `http.server.request.duration`, and answers `GET /perf/baggage`. Before traffic both products probe it; only a verified echo from a generator that sends the phase (not a real browser, not a diagnostic workload) makes capture select `perf_phase="measure"` on the request-duration metric, Tempo and Loki, and a remote target without the v1 run-id contract becomes run-isolated for traces and logs. The probe never fails a run; handed-off work keeps what the application forwards (native `harness/core/lib/baggage-contract-test.sh`, `harness/adapters/loadgen/k6/baggage-test.sh`, `harness/core/capture/capture-evidence-empty-signal-test.sh` case 9; `go test ./internal/orchestrator/ -run Baggage`, `go test ./plugins/apm/grafana/ -run Scope`, `go test ./plugins/apm/grafana/ -run PhaseMatcher`, `go test ./internal/lab/ -run BaggagePhase`, `go test ./internal/lab/ -run PhaseMatcher`) |
...
| C-13 | Native Tempo unique-fill | Deferred by design: no caller requires an exact unique-count guarantee. 993 unique traces against a 1,000 maximum is not a broken result; the returned count and dedup facts are preserved |
| C-6 extra | Second data-provider family | Deferred by design: SQL plus message-queue is a portfolio choice and nothing requires a second family |
...
| 16 | Remaining Gate C diagnostics and provenance | D-P1-3, D-P1-4, D-P1-8, D-P1-10, D-P2-7, D-P2-* remainder, C-13 | C | Done: D-P1-3, D-P1-8, D-P1-10, D-P2-3, D-P2-4 and D-P2-7 landed with proof commands; D-P1-4 is documented as unsupported; C-13 is deferred by design |
